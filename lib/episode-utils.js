...
export function getMergedGuests(ep) {
  return normalizeGuestList(mergeStringArrays(tryJson(ep?.guests_json), tryJson(ep?.manual_guests_json)));
}

export function getMergedTopics(ep) {
  return normalizeTopicList(mergeStringArrays(tryJson(ep?.topics_json), tryJson(ep?.manual_topics_json)));
}
...
export function mergeEpisodeCommunityData(ep) {
  if (!ep) return null;
  const filmTitle = getEffectiveFilmTitle(ep);
  return {
    ...ep,
    format_name: ep.format_name || detectEpisodeFormat(ep.title),
    film_title: filmTitle,
    work_id: getWorkIdFromFilmTitle(filmTitle),
    guests: getMergedGuests(ep),
    topics: getMergedTopics(ep),
